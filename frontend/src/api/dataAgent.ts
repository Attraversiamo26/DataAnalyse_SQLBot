...
export const dataAgentApi = {
  ...
  getAnalysisResults: (skip: number = 0, limit: number = 100) => {
    return request.get<AnalysisResultList>('/data-agent/analysis-results', { params: { skip, limit } });
  },
  ...
  getReportTemplates: (skip: number = 0, limit: number = 100) => {
    return request.get<ReportTemplateResponse[]>('/data-agent/report-templates', { params: { skip, limit } });
  },

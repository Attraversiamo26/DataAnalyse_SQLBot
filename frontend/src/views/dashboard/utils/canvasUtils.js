...
export const load_resource_prepare = (params, callBack) => {
    dashboardApi
...
export const isMainCanvas = (canvasId) => {
    return canvasId === 'canvas-main';
};
export const initCanvasData = (params, callBack) => {
    load_resource_prepare(params, function (result) {
...
export const findNextComponentIndex = async (params, callBack) => {
    load_resource_prepare(params, function (result) {
        let bottomPosition = 0;
        const { dashboardInfo, canvasDataResult, canvasStyleResult, canvasViewInfoPreview } = result;
        canvasDataResult.forEach((component) => {
...
// eslint-disable-next-line @typescript-eslint/no-unsafe-function-type
export const saveDashboardResource = (params, callBack) => {
    const commonParams = {
...
// eslint-disable-next-line @typescript-eslint/no-unsafe-function-type
export const saveDashboardResourceTarget = (params, commonParams, callBack) => {
    params['workspace_id'] = workspace_id;
    dashboardApi.check_name(params).then((resCheck) => {
        if (resCheck) {
            if (params.opt === 'newLeaf') {
                ...
                dashboardApi.create_canvas(requestParams).then((res) => {
                ...
                dashboardApi.create_resource(params).then((res) => {
                    callBack(res);
                });
            }
            else if (params.opt === 'rename') {
                dashboardApi.update_resource(params).then((res) => {
                    callBack(res);
                });
            }
                ...
                dashboardApi.update_canvas(requestParams).then((res) => {
                    callBack(res);
                });
            }
        }

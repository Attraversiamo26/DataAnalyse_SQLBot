import { ref, nextTick, toRefs, onMounted, getCurrentInstance } from 'vue';
...
const { canvasComponentData, baseWidth, baseHeight, baseMarginLeft, baseMarginTop, draggable, resizable, } = toRefs(props);
// DOM ref
...
function emptyTargetCell(item) {
    ...
    belowItems.forEach((downItem) => {
        if (downItem._dragId === item._dragId)
            return;
...
function startResize(e, point, item, index) {
    if (!resizable.value)
        return;
...
function getNowPosition(addSizeX, addSizeY, moveXSize, moveYSize) {
...
function startMove(e, item, index) {
    ...
    if (!className.includes('dragHandle') &&
    ...
    const itemMouseMove = (e) => {
        ...
        if (resizeItem) {
            ...
            const addSizeX = moveXSize % cellWidth.value > cellWidth.value / 4
                ? parseInt(String(moveXSize / cellWidth.value + 1))
                : parseInt(String(moveXSize / cellWidth.value));
            const addSizeY = moveYSize % cellHeight.value > cellHeight.value / 4
                ? parseInt(String(moveYSize / cellHeight.value + 1))
                : parseInt(String(moveYSize / cellHeight.value));
            // Determine the resizing direction based on the coordinate points
            const { nowSizeX, nowSizeY, nowX, nowY, nowOriginWidth, nowOriginHeight, nowOriginX, nowOriginY, } = getNowPosition(addSizeX, addSizeY, moveXSize, moveYSize);
            debounce(() => {
    ...
    const itemMouseUp = () => {
        ...
        if (infoBox.value.resizeItem) {
            ...
            if (infoBox.value.resizeItem.component === 'SQTab') {
                const refTabInstance = currentInstance.refs['shape_component_' + infoBox.value.resizeItem.id][0];
...
function getItemMatrixPosition(item) {
    return {
        tw: item.sizeX,
        th: item.sizeY,
        tl: item.x - 1,
...
// Get style location
function getItemStylePosition(item) {
...
__VLS_asFunctionalElement(__VLS_intrinsicElements.div, __VLS_intrinsicElements.div)({
    ...{ onClick: (__VLS_ctx.containerClick) },
    ...{ onMousedown: (...[$event]) => {
            __VLS_ctx.containerMouseDown($event);
        } },
    ...{ onMouseup: (...[$event]) => {
            __VLS_ctx.endMove();
        } },
...
if (!__VLS_ctx.canvasComponentData.length && __VLS_ctx.isMainCanvas(__VLS_ctx.canvasId)) {
    /** @type {[typeof EmptyBackground, ]} */ ;
    // @ts-ignore
    const __VLS_0 = __VLS_asFunctionalComponent(EmptyBackground, new EmptyBackground({
        description: (__VLS_ctx.t('dashboard.add_component_tips')),
    }));
    const __VLS_1 = __VLS_0({
        description: (__VLS_ctx.t('dashboard.add_component_tips')),
    }, ...__VLS_functionalComponentArgsRest(__VLS_0));
}
if (__VLS_ctx.renderOk) {
    for (const [item, index] of __VLS_getVForSourceType((__VLS_ctx.canvasComponentData))) {
        /** @type {[typeof CanvasShape, typeof CanvasShape, ]} */ ;
        // @ts-ignore
        const __VLS_3 = __VLS_asFunctionalComponent(CanvasShape, new CanvasShape({
            ...{ 'onEnlargeView': {} },
            key: ('item' + index),
            canEdit: (!__VLS_ctx.fullscreenFlag),
            active: (__VLS_ctx.curComponentId === item.id),
            configItem: (item),
            draggable: (__VLS_ctx.draggable),
            itemIndex: (index),
            moveAnimate: (__VLS_ctx.moveAnimate),
            startMove: (__VLS_ctx.startMove),
            startResize: (__VLS_ctx.startResize),
        ...
        let __VLS_8;
        const __VLS_9 = {
            onEnlargeView: (() => __VLS_ctx.enlargeView(item.id))
        };
        __VLS_5.slots.default;
        const __VLS_10 = ((__VLS_ctx.findComponent(item.component)));
        // @ts-ignore
        const __VLS_11 = __VLS_asFunctionalComponent(__VLS_10, new __VLS_10({
            ...
            configItem: (item),
            viewInfo: (__VLS_ctx.canvasViewInfo[item.id]),
            canvasViewInfo: (__VLS_ctx.canvasViewInfo),
            showPosition: ('canvas'),
            disabled: (__VLS_ctx.fullscreenFlag),
        }));
        ...
        let __VLS_16;
        const __VLS_17 = {
            onParentAddItemBox: ((subItem) => __VLS_ctx.addItemBox(subItem))
        };
        var __VLS_13;
        var __VLS_5;
    }
}
/** @type {__VLS_StyleScopedClasses['dragAndResize']} */ ;
/** @type {__VLS_StyleScopedClasses['sq-component']} */ ;
/** @type {__VLS_StyleScopedClasses['slot-component']} */ ;
/** @type {__VLS_StyleScopedClasses['sq-component-hidden']} */ ;
var __VLS_dollars;

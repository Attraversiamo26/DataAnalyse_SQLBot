...
import dashboard from '@/assets/svg/menu/icon_embedded_filled.svg';
import { useEmitt } from '@/utils/useEmitt';
import noDashboard from '@/assets/svg/menu/icon_dashboard_outlined.svg';
import ds from '@/assets/svg/menu/icon_log_filled.svg';
...
const MenuItem = defineComponent({
    ...
    setup(props) {
        ...
        return () => {
            ...
            const iconCom = iconMap[icon] ? ElIcon : null;
            const menuContent = [h('span', null, { default: () => title })];
            if (iconCom && iconMap[icon]) {
                menuContent.unshift(h(iconCom, { size: 18, class: 'ed-icon' }, {
                    default: () => h(iconMap[icon]),
                }));
            }
            return h(ElMenuItem, { index: path, onClick: (e) => handleMenuClick(e) }, {
                default: () => menuContent,
            });
        };
    },

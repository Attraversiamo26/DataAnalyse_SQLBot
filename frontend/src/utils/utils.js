...
export const toLoginPage = (fullPath) => {
...
export const toLoginSuccess = (router) => {
...
export const setTitle = (title) => {
    document.title = title || '邮政数据分析智能体';
};
...
export function colorStringToHex(colorStr) {
    if (colorStr.startsWith('#'))
        return colorStr;
    // 提取颜色值
    const rgbRegex = /^(rgb|rgba)\(\s*(\d{1,3})\s*,\s*(\d{1,3})\s*,\s*(\d{1,3})\s*(?:,\s*(\d+(?:\.\d+)?)\s*)?\)$/;
...
export const setCurrentColor = (color, element = document.documentElement) => {
    const currentColor = colorStringToHex(color);
    if (!currentColor) {
        return;
    }
    element.style.setProperty('--ed-color-primary', currentColor);
    element.style.setProperty('--van-blue', currentColor);
    element.style.setProperty('--ed-color-primary-light-5', colorFunctions
        .mix(new colorTree('ffffff'), new colorTree(currentColor.substr(1)), { value: 40 })
        .toRGB());
    element.style.setProperty('--ed-color-primary-light-3', colorFunctions
        .mix(new colorTree('ffffff'), new colorTree(currentColor.substr(1)), { value: 15 })
        .toRGB());
    element.style.setProperty('--ed-color-primary-60', colorFunctions
        .mix(new colorTree('ffffff'), new colorTree(currentColor.substr(1)), { value: 60 })
        .toRGB());
    element.style.setProperty('--ed-color-primary-80', colorFunctions
        .mix(new colorTree('ffffff'), new colorTree(currentColor.substr(1)), { value: 80 })
        .toRGB());
    element.style.setProperty('--ed-color-primary-15-d', colorFunctions
    ...
    element.style.setProperty('--ed-color-primary-dark-2', colorFunctions
        .mix(new colorTree('000000'), new colorTree(currentColor.substr(1)), { value: 15 })
        .toRGB());
};
...
export const isPlatform = () => {
    ...
    return (!!state &&
        !!getQueryString('code') &&
        platformArray.some((item) => state.includes(item)));
};
...
export function isMobile() {
    return (navigator.userAgent.match(/(phone|pad|pod|iPhone|iPod|ios|iPad|Android|Mobile|BlackBerry|IEMobile|MQQBrowser|JUC|Fennec|wOSBrowser|BrowserNG|WebOS|Symbian|Windows Phone)/i) && !isTablet());
}
export const getSQLBotAddr = (portEnd) => {
...
export const formatArg = (text) => {
    if (!text) {
        return false;
    }
    const mappingArray = ['true', 'false', '1', '0'];
    const match = mappingArray.some((item) => {

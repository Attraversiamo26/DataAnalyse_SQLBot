...
class LLMConfig(BaseModel):
    ...
    def __hash__(self):
        try:
            if hasattr(self, 'additional_params') and isinstance(self.additional_params, dict):
                def make_hashable(obj):
                    if isinstance(obj, dict):
                        ...
                        return obj
                hashable_params = make_hashable(self.additional_params)
            else:
            ...
            # Fallback hash if something is unhashable
            return hash((
                self.model_id,
                self.model_type,
                self.model_name,
...
async def get_default_config() -> LLMConfig:
    with Session(engine) as session:
        # 直接从数据库中查询所需的字段，而不是使用AiModelDetail对象
        result = session.exec(
            select(AiModelDetail.id, AiModelDetail.base_model, AiModelDetail.api_key, AiModelDetail.api_domain, AiModelDetail.protocol, AiModelDetail.config)
            .where(AiModelDetail.default_model == True)
        ).first()
        if not result:
            raise Exception("The system default model has not been set")
        
        id, base_model, api_key, api_domain, protocol, config = result

        additional_params = {}
        if config:
            try:
                config_raw = json.loads(config)
                # 检查config_raw是否为列表，如果是则按原逻辑处理，否则直接处理单个键值对
                ...
                elif isinstance(config_raw, dict) and "key" in config_raw and "val" in config_raw:
                    additional_params = {config_raw["key"]: prepare_model_arg(config_raw.get('val'))}
            except Exception:
                pass
        ...
        return LLMConfig(
            model_id=id,
            model_type="openai" if protocol == 1 else "vllm",

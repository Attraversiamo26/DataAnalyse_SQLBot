...
from ..crud.field import get_fields_by_table_id, batch_update_field_comments
...
class BatchUpdateFieldComments(BaseModel):
    table_id: int
    field_comments: dict
...
@router.post("/batchUpdateFieldComments", response_model=dict, summary=f"{PLACEHOLDER_PREFIX}ds_batch_update_field_comments")
@require_permissions(permission=SqlbotPermission(role=['ws_admin']))
async def batch_update_field_comments_api(session: SessionDep, data: BatchUpdateFieldComments):
    """
    批量更新字段的自定义注释
    """
    def inner():
        updated_count = batch_update_field_comments(session, data.table_id, data.field_comments)

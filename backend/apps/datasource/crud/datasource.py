...
from .ws_ds import get_ws_ds, clear_ws_ds_cache
...
async def delete_ds(session: SessionDep, id: int):
    term = session.query(CoreDatasource).filter(CoreDatasource.id == id).first()
    if not term:
...
def preview(session: SessionDep, current_user: CurrentUser, id: int, data: TableObj):
    ...
    if is_normal_user(current_user):
        ...
        # Mock DsRules query to return empty list
        contain_rules = []

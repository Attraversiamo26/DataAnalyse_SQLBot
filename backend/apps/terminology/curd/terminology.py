...
def build_terminology_query(session: SessionDep, oid: int, name: Optional[str] = None,
    ...
    children_subquery = (
        select(
            child.pid,
            func.group_concat(child.word).label('other_words')
        )
    ...
    # 简化查询，移除数据源名称的获取，因为 MySQL 处理 JSON 数组的方式不同
    # 后续在 Python 代码中处理数据源名称的映射
...
def execute_terminology_query(session: SessionDep, stmt) -> List[TerminologyInfo]:
    ...
    for row in result:
        _list.append(TerminologyInfo(
            ...
            datasource_names=[],  # 简化处理，MySQL 中暂时不支持数据源名称的聚合
...
def create_terminology(session: SessionDep, info: TerminologyInfo, oid: int, trans: Trans,
    ...
    if specific_ds:
        ...
        # 构建 OR 条件，检查 datasource_ids 包含任一目标数据源
        datasource_conditions = []
...
# MySQL 不支持向量相似度计算，暂时注释掉嵌入相关的 SQL
# embedding_sql = f"""
...
#      OR
#     (specific_ds = true AND datasource_ids IS NOT NULL AND JSON_CONTAINS(datasource_ids, JSON_ARRAY(:datasource)))
# )
# ORDER BY similarity DESC
# LIMIT {settings.EMBEDDING_TERMINOLOGY_TOP_COUNT}
# """
        ...
        stmt = stmt.where(
            or_(
                ...
                and_(
                    Terminology.specific_ds == True,
                    Terminology.datasource_ids.isnot(None),
                    text("JSON_CONTAINS(datasource_ids, JSON_ARRAY(:datasource))")
                )
            )
        )
    else:

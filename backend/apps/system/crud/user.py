...
async def get_user_info(*, session: Session, user_id: int) -> UserInfoDTO | None:
    ...
    if userInfo.isAdmin:
        return userInfo
    ws_model: UserWsModel = session.exec(select(UserWsModel).where(UserWsModel.user_id == userInfo.id, UserWsModel.ws_id == userInfo.oid)).first()
...
async def user_ws_options(session: Session, uid: int, trans: Optional[I18n] = None) -> list[UserWs]:
    if uid == 1:
        ...
        stmt = select(WorkspaceModel.id, WorkspaceModel.name).join(
            UserWsModel, UserWsModel.ws_id == WorkspaceModel.id
        ).where(
            UserWsModel.user_id == uid,
...
async def single_delete(session: SessionDep, id: int):
    ...
    del_stmt = sqlmodel_delete(UserWsModel).where(UserWsModel.user_id == id)

...
async def reset_single_user_oid(session: Session, uid: int, oid: int, add: Optional[bool] = True):
    ...
    if not add and origin_oid and origin_oid == oid:
        user_model.oid = 0
        user_ws = session.exec(select(UserWsModel).where(UserWsModel.user_id == uid, UserWsModel.ws_id != oid)).first()
        if user_ws:
...
async def reset_user_oid(session: Session, oid: int):
    stmt = (
        ...
        .join(UserWsModel, UserModel.id == UserWsModel.user_id, isouter=True)

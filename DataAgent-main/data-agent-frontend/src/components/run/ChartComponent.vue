...
<script setup lang="ts">
  ...
  // 根据数据特征自动选择合适的图表类型
  const autoSelectChartType = (): ChartTypes => {
    const data = props.resultData.resultSet.data;
    const columns = props.resultData.resultSet.column;
    ...
    // 检查是否有时间序列数据
    const hasTimeData = columns.some(column => {
      const firstValue = data[0][column];
      return !isNaN(Date.parse(firstValue)) || firstValue.toString().includes('-') || firstValue.toString().includes('/');
    });
    ...
    // 根据数据特征选择图表类型
    if (columnCount === 2 && numericColumnCount === 1 && categoryColumnCount === 1) {
      // 一个分类列和一个数值列，适合饼图
      if (rowCount <= 10) {
        return 'pie';
      } else {
    ...
    } else if (hasTimeData && numericColumnCount >= 1) {
      // 有时间序列数据，适合折线图
      return 'line';
    } else if (numericColumnCount >= 2) {
      // 有多个数值列，适合散点图
      return 'scatter';
    } else if (columnCount >= 3 && categoryColumnCount >= 2 && numericColumnCount >= 1) {
      // 有多个分类列和一个数值列，适合热力图
      return 'heatmap';
    } else {
      // 默认使用柱状图
  ...
  const initChart = () => {
    ...
    // 解析图表类型，如果没有指定则自动选择
    const chartType = (props.resultData.displayStyle?.type as ChartTypes) || autoSelectChartType();

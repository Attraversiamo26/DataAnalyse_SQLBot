...
> 本文档详细介绍了 `custom_skills` 目录下所有技能的功能、数据结构、执行方式和编排工作流设计。
...
- [一、整体架构](#一整体架构)
- [二、数据源说明](#二数据源说明)
- [三、技能详细说明与工作流编排](#三技能详细说明与工作流编排)
  - [3.1 线路五环节时限对标分析](#31-线路五环节时限对标分析)
  - [3.2 出口环节下钻分析](#32-出口环节下钻分析)
  - [3.3 进口环节下钻分析](#33-进口环节下钻分析)
  - [3.4 投递环节下钻分析](#34-投递环节下钻分析)
  - [3.5 中转环节下钻分析](#35-中转环节下钻分析)
  - [3.6 收寄环节下钻分析](#36-收寄环节下钻分析)
  - [3.7 收寄机构下钻分析](#37-收寄机构下钻分析)
  - [3.8 出口机构下钻分析](#38-出口机构下钻分析)
- [四、技能依赖管理](#四技能依赖管理)
- [五、组合工作流设计](#五组合工作流设计)
- [六、工作流执行引擎](#六工作流执行引擎)
- [七、扩展开发指南](#七扩展开发指南)
...
**技能名称**: `line-five-stage-time-benchmark-analysis`
...
```yaml
workflow:
  id: line-five-stage-time-benchmark-analysis
  name: 线路五环节时限对标分析工作流
...
print(f"问题环节: {result.outputs['key_problem_stage']}")
```
...
```yaml
on_success:
  - skill: export-link-drilldown-analysis
    condition: "${outputs.key_problem_stage == '出口'}"
    params:
      route_name: "${params.route_name}"
  
  - skill: import-link-drilldown-analysis
    condition: "${outputs.key_problem_stage == '进口'}"
    params:
      route_name: "${params.route_name}"
  
  - skill: delivery-stage-drilldown-analysis
    condition: "${outputs.key_problem_stage == '投递'}"
    params:
      route_name: "${params.route_name}"
  
  - skill: transit-link-drilldown-analysis
    condition: "${outputs.key_problem_stage == '中转'}"
    params:
      route_name: "${params.route_name}"
```
...
```yaml
workflow:
  id: export-link-drilldown-analysis
  name: 出口环节下钻分析工作流
...
curl -X POST http://localhost:8001/api/workflow/execute \
  -H "Content-Type: application/json" \
  -d '{
    "workflow_id": "export-link-drilldown-analysis",
    "params": {
      "route_name": "滨州市到大同市",
      "top_n": 10
    }
...
result = engine.execute(
    workflow_id="export-link-drilldown-analysis",
    params={"route_name": "滨州市到大同市", "top_n": 10}
)

for inst in result.outputs['slow_institutions']:
    print(f"{inst['机构名称']}: {inst['出口-处理时限']}小时")
```
...
```yaml
on_success:
  - skill: export-institution-drilldown-analysis
    condition: "${user_confirm == true}"
    params:
...
**技能名称**: `import-link-drilldown-analysis`
...
```yaml
workflow:
  id: import-link-drilldown-analysis
  name: 进口环节下钻分析工作流
...
curl -X POST http://localhost:8001/api/workflow/execute \
  -H "Content-Type: application/json" \
  -d '{
    "workflow_id": "import-link-drilldown-analysis",
    "params": {
      "route_name": "滨州市-惠州市"
    }
  }'
...
result = engine.execute(
    workflow_id="import-link-drilldown-analysis",
    params={"route_name": "滨州市-惠州市"}
)

print(f"Word报告: {result.outputs['docx_report']}")
print(f"PDF报告: {result.outputs['pdf_report']}")
print(f"分布图: {result.outputs['charts']}")
```
...
**技能名称**: `delivery-stage-drilldown-analysis`
...
```yaml
workflow:
  id: delivery-stage-drilldown-analysis
  name: 投递环节下钻分析工作流
...
curl -X POST http://localhost:8001/api/workflow/execute \
  -H "Content-Type: application/json" \
  -d '{
    "workflow_id": "delivery-stage-drilldown-analysis",
    "params": {
      "route_name": "滨州市-惠州市"
    }
  }'
...
**技能名称**: `transit-link-drilldown-analysis`
...
```yaml
workflow:
  id: transit-link-drilldown-analysis
  name: 中转环节下钻分析工作流
...
curl -X POST http://localhost:8001/api/workflow/execute \
  -H "Content-Type: application/json" \
  -d '{
    "workflow_id": "transit-link-drilldown-analysis",
    "params": {
      "route_name": "滨州市-惠州市"
    }
  }'
...
```yaml
workflow:
  id: pickup-stage-drilldown-analysis
  name: 收寄环节下钻分析工作流
  ...
  on_user_confirm:
    ...
    trigger:
      - skill: pickup-institution-drilldown-analysis
        params:
          institution_name: "${outputs.slowest_institution}"
          route_name: "${params.route_name}"
```
...
curl -X POST http://localhost:8001/api/workflow/execute \
  -H "Content-Type: application/json" \
  -d '{
    "workflow_id": "pickup-stage-drilldown-analysis",
    "params": {
      "route_name": "滨州市-大同市",
      "top_n": 5
    }
...
result = engine.execute(
    workflow_id="pickup-stage-drilldown-analysis",
    params={"route_name": "滨州市-大同市", "top_n": 5}
)

# 如果用户确认，自动触发机构下钻
if result.outputs.get('user_confirmed'):
    drilldown_result = engine.execute(
        workflow_id="pickup-institution-drilldown-analysis",
        params={
            "institution_name": result.outputs['slowest_institution'],
            "route_name": "滨州市-大同市"
        }
    )
```
...
```yaml
workflow:
  id: pickup-institution-drilldown-analysis
  name: 收寄机构下钻分析工作流
...
```yaml
workflow:
  id: export-institution-drilldown-analysis
  name: 出口机构下钻分析工作流
...
curl -X POST http://localhost:8001/api/workflow/execute \
  -H "Content-Type: application/json" \
  -d '{
    "workflow_id": "export-institution-drilldown-analysis",
    "params": {
      "institution_name": "济南齐河",
      "route_name": "滨州市东莞市"
    }
  }'
```
...
| 技能 | 额外依赖 | 用途 |
|------|----------|------|
| pickup-institution-drilldown-analysis | pdfkit | PDF 生成 |
| export-institution-drilldown-analysis | pdfkit | PDF 生成 |
...
```yaml
workflow:
  id: full-route-analysis
  name: 完整线路分析工作流
  description: 从全景到机构的完整分析流程
  ...
  steps:
    - id: phase_1_overview
      name: 第一阶段-全景分析
      workflow: line-five-stage-time-benchmark-analysis
      params:
        route_name: "${params.route_name}"
      outputs:
        - key_problem_stage
    ...
    - id: phase_2_stage_analysis
      name: 第二阶段-环节分析
      parallel: true
      workflows:
        - workflow: export-link-drilldown-analysis
          condition: "${phase_1_overview.key_problem_stage == '出口'}"
          params:
            route_name: "${params.route_name}"
            top_n: 5
        
        - workflow: import-link-drilldown-analysis
          condition: "${phase_1_overview.key_problem_stage == '进口'}"
          params:
            route_name: "${params.route_name}"
        
        - workflow: delivery-stage-drilldown-analysis
          condition: "${phase_1_overview.key_problem_stage == '投递'}"
          params:
            route_name: "${params.route_name}"
        
        - workflow: transit-link-drilldown-analysis
          condition: "${phase_1_overview.key_problem_stage == '中转'}"
          params:
            route_name: "${params.route_name}"
    
    - id: phase_3_institution_drilldown
      name: 第三阶段-机构下钻
      condition: "${user_confirm == true}"
      workflows:
        - workflow: pickup-stage-drilldown-analysis
          params:
            route_name: "${params.route_name}"
            top_n: 5
          on_success:
            - workflow: pickup-institution-drilldown-analysis
              params:
                institution_name: "${outputs.slowest_institution}"
                route_name: "${params.route_name}"
  ...
  outputs:
    - name: overview_report
      value: "${phase_1_overview.ppt_report}"
    - name: stage_reports
      value: "${phase_2_stage_analysis.reports}"
    - name: institution_reports
      value: "${phase_3_institution_drilldown.reports}"
```
...
workflow:
  ...
  steps:
    ...
    - id: aggregate_results
      ...
      config:
        sources:
          - "${parallel_analysis.export-link-drilldown-analysis}"
          - "${parallel_analysis.import-link-drilldown-analysis}"
          - "${parallel_analysis.delivery-stage-drilldown-analysis}"
          - "${parallel_analysis.transit-link-drilldown-analysis}"
        output_format: "summary_report"
  
  outputs:
    - name: summary_report
      value: "${aggregate_results.summary_report}"
```
...
class WorkflowEngine:
    ...
    async def execute(self, workflow_id: str, params: dict) -> WorkflowResult:
        """执行工作流"""
        workflow_def = self._load_workflow(workflow_id)
        self.context = {"params": params, "outputs": {}}
    ...
    async def _execute_step(self, step: dict):
        ...
        self.context["outputs"][step["id"]] = result
    
    async def _execute_parallel(self, step: dict):
        """并行执行多个步骤"""
        tasks = []
        for sub_step in step.get("workflows", step.get("skills", [])):
            if self._check_condition(sub_step):
                task = self._execute_step(sub_step)
                tasks.append(task)
        
        results = await asyncio.gather(*tasks)
        self.context["outputs"][step["id"]] = results
    ...
    def _resolve_params(self, params: dict) -> dict:
        ...
        for key, value in params.items():
            if isinstance(value, str) and value.startswith("${"):
                resolved[key] = self._evaluate_expression(value)
            else:
                resolved[key] = value
        return resolved
...
class SkillRegistry:
    ...
    def _load_skills(self):
        ...
        for skill_dir in skills_dir.iterdir():
            if skill_dir.is_dir() and (skill_dir / "SKILL.md").exists():
                ...
                self.skills[metadata["name"]] = SkillExecutor(
                    metadata=metadata,
                    skill_dir=skill_dir
                )
    ...
    async def execute(self, skill_id: str, params: dict) -> dict:
        """执行技能"""
        executor = self.skills.get(skill_id)
        if not executor:
...
async def list_skills():
    """列出所有技能"""
    return engine.skill_registry.list_skills()
```
...
在 `workflows/` 目录下创建 `new-skill-name.yaml`

### 7.2 新增组合工作流

在 `workflows/` 目录下创建新的 YAML 文件：
...
workflow:
  ...
  steps:
    - id: step_1
      workflow: existing-skill-1
      params:
        key: "${params.param1}"
    
    - id: step_2
      workflow: existing-skill-2
      params:
        key: "${step_1.outputs.result}"
```
...
| 技能名称 | 语言 | 层级 | 输入 | 输出 | 工作流ID |
|----------|------|------|------|------|----------|
| line-five-stage-time-benchmark-analysis | Node.js | 全景 | 线路名 | PPT, PDF | line-five-stage-time-benchmark-analysis |
| export-link-drilldown-analysis | Node.js | 环节 | 线路名, TopN | 控制台 | export-link-drilldown-analysis |
| import-link-drilldown-analysis | Python | 环节 | 线路名 | DOCX, PDF, PNG | import-link-drilldown-analysis |
| delivery-stage-drilldown-analysis | Python | 环节 | 线路名 | DOCX, PDF, PNG | delivery-stage-drilldown-analysis |
| transit-link-drilldown-analysis | Python | 环节 | 线路名 | DOCX, PDF | transit-link-drilldown-analysis |
| pickup-stage-drilldown-analysis | Node.js | 环节 | 线路名, TopN | 控制台 | pickup-stage-drilldown-analysis |
| pickup-institution-drilldown-analysis | Node.js | 机构 | 机构名, 线路名 | PDF | pickup-institution-drilldown-analysis |
| export-institution-drilldown-analysis | Node.js | 机构 | 机构名, 线路名 | PDF | export-institution-drilldown-analysis |
...
curl -X POST http://localhost:8001/api/workflow/execute \
  -H "Content-Type: application/json" \
  -d '{"workflow_id": "line-five-stage-time-benchmark-analysis", "params": {"route_name": "滨州市-晋城市"}}'

# 执行完整分析工作流
curl -X POST http://localhost:8001/api/workflow/execute \
  -H "Content-Type: application/json" \
  -d '{"workflow_id": "full-route-analysis", "params": {"route_name": "滨州市-晋城市"}}'

# 执行快速诊断工作流
curl -X POST http://localhost:8001/api/workflow/execute \
  -H "Content-Type: application/json" \
  -d '{"workflow_id": "quick-diagnosis", "params": {"route_name": "滨州市-晋城市"}}'
```

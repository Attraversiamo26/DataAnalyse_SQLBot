...
def main():
    ...
    # 标题
    title = doc.add_heading('滨州市晋城市 线路投递环节时限对标下钻分析报告', 0)
    ...
    para1 = doc.add_paragraph('以滨州市至晋城市线路为例，特快及竞品主要轨迹相同（滨州市-济南市-太原市-晋城市），特快全程时限为51.8小时，竞品为48.4小时，特快比竞品慢3.4小时。')
    ...
    para2 = doc.add_paragraph('投递环节方面，特快投递时限为3.5小时，竞品为5.7小时，特快比竞品快2.2小时。投递环节特快表现优异，是邮政的竞争优势环节。')
    ...
    # 获取end5机构
    route_delivery = df_delivery[df_delivery['线路名'] == '滨州市晋城市'].copy()
    if not route_delivery.empty:
        ...
        summary = doc.add_paragraph('滨州市晋城市线路特快全程时限为51.8小时，竞品为48.4小时，特快比竞品慢3.4小时。特快及竞品主要轨迹相同，均为滨州市-济南市-太原市-晋城市。投递环节特快投递时限为3.5小时，竞品为5.7小时，特快比竞品快2.2小时，投递环节是邮政的竞争优势。但景西大宗等个别机构存在妥投时间过长问题，需要针对性优化。')
    ...
    output_path = os.path.join(output_dir, '线路投递环节时限分析报告_滨州市晋城市_{}.docx'.format(timestamp))
    doc.save(output_path)
    print('Word文档已保存: ' + output_path)
    
    # 显示48小时分布图路径
    chart_files = [f for f in os.listdir(output_dir) if f.startswith('48h_景西大宗') and f.endswith('.png')]
    if chart_files:
